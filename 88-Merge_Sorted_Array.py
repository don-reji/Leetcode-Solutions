...
# Pointer to place the next largest element at the last position in nums1
    last = m + n - 1  
    ...
    while m > 0 and n > 0:
        ...
        if nums1[m - 1] > nums2[n - 1]:  
            # Place the larger element at the last position
        ...
        last -= 1  # Move the last position pointer backward

    # If nums2 still has elements left, copy them to nums1
    while n > 0:
        nums1[last] = nums2[n - 1]
        n -= 1
        last -= 1  # Move last position pointer backward
